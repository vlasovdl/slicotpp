...
public:
  ...
  /// @note Without argument (or when rows == 0) it will be creating an
  /// indefinite size matrix. One should be filing data by insert_data() method,
  /// or by join_column() method. When rows > 0 and cols is absent it will be
  /// a vector. When size is defined (matrix or vector) it filled default value
  /// of type value_t.
  ...
  value_type& operator()(const int row, const int col) {
    // When rows_ and cols_ are zero, it's mean that the matrix is not a
    // definitely size. In that case impossible to get data from there. Further,
    // indexes of element should be in range of matrix sizes.
  ...
  /// @note Inserting data does not set or change the matrix size. The inserted
  /// data just placed in storage. The matrix size must then be set by resize()
  /// method.
  ...
  /// data just placed in storage. The matrix size must then be set by resize()
  /// method.
  /// todo: This method should be refactored for using with arbitrary suitable
  ///       container
  ...
  /// Insert data of convertible type from range to matrix. With iterator
  /// decrement.
  ...
  /// @note In case when the matrix is undefined size join a column will make a
  /// vector of same size as vector being merged. If matrix already has size,
  /// then it is possible to join only vector with size equal to @memberof rows().
  template<typename T>
...
using fd_matrix = f_matrix<double>;
using fi_matrix = f_matrix<int>;
